...
neko = [[0 for i in range(8)] for i in range(10)]
check = [[0 for i in range(8)] for i in range(10)]
...
def check_neko():
    ...
    for y in range(1, 9):
        for x in range(8):
            if check[y][x] > 0:
                if check[y - 1][x] == check[y][x] and check[y + 1][x]:
                    neko[y - 1][x] = 7
                    neko[y][x] = 7
                    neko[y + 1][x] = 7
...
def draw_txt(txt, x, y, siz, col, tg):
    ...
    cvs.create_text(x, y, text=txt, fill="black", font=fnt, tag=tg)
...
def game_main():
    ...
    if index == 0: # 타이틀 로고
        draw_txt("야옹야옹", 312, 240, 100, "violent", "TITLE")
        ...
        if 24 <= mouse_x < 24 + 72 * 8 \
                and 24 <= mouse_y < 24 + 72 * 10:
    ...
    draw_txt(f"HISC {hisc}", 450, 60, 32, "yellow", "INFO")
